    ...
    private fun getRawBaseTaskData(category: BaseCategoryEnum): List<Triple<Int, Int, TaskSchedule>> {
        return when (category) {
            BaseCategoryEnum.FACE -> listOf(
                Triple(R.string.base_face_category_task_day_cream, 2, TaskSchedule(1, TaskFrequency.DAY)),
                Triple(R.string.base_face_category_task_night_cream, 2, TaskSchedule(1, TaskFrequency.DAY)),
                Triple(R.string.base_face_category_task_serum, 2, TaskSchedule(4, TaskFrequency.DAY)),
            )
            BaseCategoryEnum.HANDS -> listOf(
                Triple(R.string.base_hands_category_task_cream, 1, TaskSchedule(1, TaskFrequency.DAY)),
                Triple(R.string.base_hands_category_task_massage, 1, TaskSchedule(2, TaskFrequency.WEEK)),
                Triple(R.string.base_hands_category_task_manicure, 2, TaskSchedule(1, TaskFrequency.MONTH))
            )
            BaseCategoryEnum.HAIR -> listOf(
                Triple(R.string.base_hair_category_task_other, 1, TaskSchedule(1, TaskFrequency.WEEK))
            )
            BaseCategoryEnum.BODY -> listOf(
                Triple(R.string.base_body_category_task_cream, 1, TaskSchedule(3, TaskFrequency.WEEK)),
                Triple(R.string.base_body_category_task_massage, 1, TaskSchedule(4, TaskFrequency.WEEK))
            )
            BaseCategoryEnum.FEET -> listOf(
                Triple(R.string.base_feet_category_task_other, 1, TaskSchedule(2, TaskFrequency.WEEK))
            )
            BaseCategoryEnum.ORAL -> listOf(
                Triple(R.string.base_oral_category_task_dentist, 3, TaskSchedule(2, TaskFrequency.YEAR)),
                Triple(R.string.base_oral_category_task_other, 1, TaskSchedule(2, TaskFrequency.WEEK))
            )
            BaseCategoryEnum.OTHER -> listOf(
                Triple(R.string.base_other_category_task_other, 1, TaskSchedule(2, TaskFrequency.WEEK))
            )
        }
    }
